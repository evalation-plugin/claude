...
function packageAdvice(group, plans, { upgradeTo, compared, compatible }) {
  ...
  // Worded by how many copies there are. With one, the upgrade is the sentence, and "every copy" would
  // tell a reader there are others somewhere.
  ...
  const known = group.results.every((one) => one.relation);
  const said = [];
  if (!single) said.push(`Bring ${versions.length === 2 ? "both" : `all ${versions.length}`} copies of ${name}, ${versions.join(" and ")}, to ${target}, which fixes ${fixes}.`);
  else if (!known || group.results.every((one) => one.relation === "direct")) said.push(`Upgrade ${name} from ${versions[0]} to ${target}, which fixes ${fixes}.`);
  else said.push(`Version ${target} fixes ${fixes}.`);
  ...
  if (!known) {
    if (!single) {
      said.push(`Another package likely brings one of them in${why ? `, and ${why} shows which` : ""}. Upgrade the repository's own dependency, and upgrade or override whatever brings in the other.`);
    } else if (why) {
      said.push(`Where it is not the repository's own dependency, ${why} shows which package brings it in.`);
    }
    said.push(...(single ? [breaking(versions[0]).trim()].filter(Boolean) : []));
  } else {
    ...
    for (const [version, results] of [...byVersion].sort((a, b) => Number(own(b[1])) - Number(own(a[1])))) {
      if (own(results)) {
        if (single) said.push(breaking(version).trim());
        else said.push(`Upgrade the repository's own copy, ${version}.${breaking(version)}`);
        continue;
      }
      const via = [...new Set(results.flatMap((one) => one.via ?? []))];
      ...
      said.push(planned.length > 0
        ? `${single ? `${name} ${version}` : version} comes in through ${via.join(", ")}, whose own ${planned.length === 1 ? "card already calls" : "cards already call"} for upgrading ${planned.join(" and ")}. Make that upgrade first, then${why ? ` run ${why} to` : ""} check whether a copy below ${target} is left, and only if one is, force it with ${force}.`
        : `${single ? `${name} ${version}` : version} comes in through ${via.length > 0 ? via.join(", ") : "another package"}: upgrade ${via.length === 1 ? "it" : "them"} to a release that uses ${name} ${target} or later, and if none does yet, force it with ${force}.`);
...
// What to do about each thing git history measures, by the measure's key.
const OWNED = "Have a second person review and take part in the changes to each main area, name an owner " +
  "for each in a code owners file, and write down how to build and deploy, so the work does not rest on one person.";
const HISTORY_TODO = {
  concentration: OWNED,
  area: OWNED,
  releases: "Tag each release in git with its version, and release on a regular schedule, so anyone can see what shipped and when.",
  dormant: "Check each named area is still used. Remove what is not, and give what is an owner who keeps it current.",
  slowing: "Check whether the drop in changes was planned. Where it was not, it can mean fewer people now work on the code.",
  quiet: "Check whether the code is still maintained. A year with no changes leaves known advisories and fixes unapplied.",
};

function scanResults({ findings, intro, tagWord, tagOf, phaseOf, consequences, remedies, upgradeTo, compared, compatible, cardOf }) {
  ...
  const card = (group) => {
    ...
    if (group.phase === "sca") {
      ...
      todo = HISTORY_TODO[String(group.results[0].key).split(":")[1]] ?? HISTORY_TODO.concentration;
    } else {
