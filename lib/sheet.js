...
function scanTable({ findings, intro, last, lastOf, phaseOf, whereOf }) {
  ...
  const row = (one) => {
    ...
    return `<div class="scanrow" data-lens="Scan findings"><span class="sev" style="color:${colour}">${escaped(one.severity)}</span>` +
  ...
  const opening = `<div class="block lens" data-lens="Scan findings"><h2>Scan findings (${sorted.length})</h2>` +
    `<p class="blurb">${escaped(intro)}</p>${first ? head + row(first).replace(' data-lens="Scan findings"', "") : ""}</div>`;
