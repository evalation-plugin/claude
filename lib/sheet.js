...
function scanResults({ findings, intro, tagWord, tagOf, phaseOf, consequences, remedies, upgradeTo, compared, compatible, cardOf }) {
  ...
  const card = (group) => {
    ...
    if (group.phase === "sca") {
      ...
      todo = "Have a second person review and take part in the changes to each main area, name an owner " +
        "for each in a code owners file, and write down how to build and deploy, so the work does not rest " +
        "on one person.";
    } else {
