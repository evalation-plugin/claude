 ...
 */
// How a package manager forces one version of a dependency everywhere, by the lockfile it keeps.
const OVERRIDE = [
  [/(^|\/)pnpm-lock\.yaml$/, (name, to) => `an override in package.json, "pnpm": { "overrides": { "${name}": "^${to}" } }`],
  [/(^|\/)package-lock\.json$/, (name, to) => `an override in package.json, "overrides": { "${name}": "^${to}" }`],
  [/(^|\/)yarn\.lock$/, (name, to) => `a resolution in package.json, "resolutions": { "${name}": "^${to}" }`],
  [/(^|\/)Cargo\.lock$/, (name, to) => `cargo update -p ${name} --precise ${to}, where the crates that use it allow that version`],
...
/**
 * What to do about one package: the version that clears every advisory against every copy of it, and
 * for each copy another package brings in, which package that is and how to force the version where
 * upgrading that package does not reach it. Upgrading the repository's own dependency leaves a copy
 * another package pins exactly where it was.
 */
function packageAdvice(group, upgradeTo, compared) {
  const name = group.results[0].package;
  ...
  const brought = [...byVersion].filter(([, results]) => results.some((one) => one.relation === "indirect"));
  const own = [...byVersion].filter(([, results]) => results.every((one) => one.relation === "direct"));
  if (own.length > 0) said.push(`Upgrade the repository's own dependency on ${name}${own.length === 1 ? `, now ${own[0][0]}` : ""}.`);
  for (const [version, results] of brought) {
    const via = [...new Set(results.flatMap((one) => one.via ?? []))];
    const lockfile = [...new Set(results.map((one) => one.at?.path ?? ""))][0] ?? "";
    const force = OVERRIDE.find(([pattern]) => pattern.test(lockfile))?.[1](name, target) ?? "your package manager's override for it";
    said.push(`${version} comes in through ${via.length > 0 ? via.join(", ") : "another package"}: upgrade ${via.length === 1 ? "it" : "them"} to a release using ${name} ${target} or later, or force the version with ${force}.`);
  }
  if (unfixed > 0) said.push(`No published version fixes the other ${unfixed}, so reduce what reaches ${name} or replace it.`);
  return said.join(" ");
}

function scanResults({ findings, intro, tagWord, tagOf, phaseOf, consequences, remedies, upgradeTo, compared }) {
  ...
  const card = (group) => {
    ...
    let todo;
    if (group.phase === "sca") {
      todo = packageAdvice(group, upgradeTo, compared);
    } else {
    ...
    const opening = `<article class="entry opens" data-lens="Scanner results"><div class="head">` +
      ...
      (group.phase === "sca" ? "" : (group.results[0].body ? `<p class="because" data-quoted>${escaped(group.results[0].body)}</p>` : "")) +
