...
/* The scan table. Each row is a block of its own on one grid, so rows line up down a page and across
   pages, and a table longer than a page carries its header onto the next. */
.scanrow{display:grid;grid-template-columns:0.72in 1fr 1.65in 1.0in 0.95in 0.9in;gap:0.08in;
  padding:4px 8px;border-bottom:1px solid ${CHROME.hairline};font-size:7.5pt;line-height:10pt;
  break-inside:avoid}
.scanrow span{overflow-wrap:anywhere}
.scanrow .mono{font-family:'JetBrains Mono',monospace;font-size:7pt;color:${CHROME.muted}}
.scanrow.head{background:${CHROME.inset};font-family:'JetBrains Mono',monospace;font-size:6.5pt;
  letter-spacing:0.08em;text-transform:uppercase;color:${CHROME.faint}}
.scanrow .sev{font-family:'JetBrains Mono',monospace;font-size:7pt;text-transform:uppercase;letter-spacing:0.05em}
.scanrow .what{grid-column:2 / -1;color:${CHROME.muted};font-size:7pt;line-height:9.5pt}`;
}
...
/**
 * Every scanner finding a report bears on, worst first, as a table that runs on across pages. Each
 * row is its own block, so the paginator moves rows and never has to place a table taller than a
 * page, and the header row repeats at the top of each page it continues onto. The last column is
 * the report's own: the clauses a finding bears on, or the finding it was reported as.
 */
function scanTable({ findings, intro, last, lastOf, phaseOf, whereOf }) {
  // One row per result. A scanner reports an advisory once for each lockfile that carries the
  // package, and the same weakness listed five times reads as five.
  const byKey = new Map();
  for (const one of findings) {
    if (!byKey.has(one.key)) byKey.set(one.key, { ...one, places: new Set() });
    byKey.get(one.key).places.add(one.at?.path ?? "");
  }
  findings = [...byKey.values()];
  // A table of nothing says nothing, so a report with no scanner results to list carries none.
  if (findings.length === 0) return "";
  const rank = (one) => { const at = SCAN_SEVERITY.findIndex(([name]) => name === one.severity); return at < 0 ? SCAN_SEVERITY.length : at; };
  const sorted = [...findings].sort((a, b) => rank(a) - rank(b) || String(a.key).localeCompare(String(b.key)));
  const head = `<div class="scanrow head scanhead"><span>Severity</span><span>Finding</span><span>Where</span>` +
    `<span>Fixed in</span><span>Scan</span><span>${escaped(last)}</span></div>`;
  const row = (one) => {
    const colour = SCAN_SEVERITY.find(([name]) => name === one.severity)?.[1] ?? CHROME.faint;
    return `<div class="scanrow" data-lens="Scanner results"><span class="sev" style="color:${colour}">${escaped(one.severity)}</span>` +
      // The advisory or rule alone. A scanner's title goes on to say where, which the next column says.
      `<span>${escaped(String(one.title ?? one.key).split(" in ")[0])}</span>` +
      `<span class="mono">${escaped(whereOf(one) + (one.places.size > 1 ? `, in ${one.places.size} places` : ""))}</span>` +
      `<span class="mono">${escaped(one.fixed ?? "")}</span><span>${escaped(phaseOf(one.phase))}</span>` +
      `<span class="mono">${escaped(lastOf(one))}</span>` +
      // What the weakness is and what it lets somebody do, in the scanner's words, across the row.
      (one.body ? `<span class="what">${escaped(one.body)}</span>` : "") + `</div>`;
  };
  const [first, ...rest] = sorted;
  const opening = `<div class="block lens" data-lens="Scanner results"><h2>Scanner results (${sorted.length})</h2>` +
    `<p class="blurb">${escaped(intro)}</p>${first ? head + row(first).replace(' data-lens="Scanner results"', "") : ""}</div>`;
  return opening + rest.map(row).join("");
}
...
(function () {
  ...
  function carry(block) {
    ...
    // A table row continuing onto a fresh sheet brings the table's header row with it.
    if (block.classList.contains('scanrow')) {
      var head = out.querySelector('.scanhead');
      if (head) flow.appendChild(head.cloneNode(true));
    }
  }
...
module.exports = { CHROME, PAPER, css, escaped, faces, mark, paginator, scanTable };
