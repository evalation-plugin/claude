...
function summary(held) {
  ...
  const risk = sentences(
    ...
    counts.Critical
      ? `${counts.Critical} ${plural(counts.Critical, "finding")} ${counts.Critical === 1 ? "is" : "are"} ` +
        `critical, meaning the control is absent rather than thin, and ${counts.Critical === 1 ? "it sets" : "they set"} ` +
    ...
    counts.High
      ...
      : "",
    depth
      ? `The remaining ${depth} are depth, and none of them changes what the ` +
        "system guarantees."
      : "",
  );
  ...
  const takes = sentences(
    serious
      ? `Closing the ${severe(counts)} ${plural(serious, "finding")} clears the material risk. ` +
  ...
  const bottom = sentences(
    ...
    serious
      ? `The ${severe(counts)} ${plural(serious, "finding")} ${serious === 1 ? "carries" : "carry"} the risk. ` +
        "The rest raise the floor."
  ...
  const lead = sentences(
    ...
    "anything.",
    serious
      ? `${counts.Critical ? "" : "Nothing critical was found. "}The ${severe(counts)} ` +
        `${plural(serious, "finding")} ${serious === 1 ? "is" : "are"} the work that changes the risk.`
    ...
    body: trimmed(sentences(
      ...
      clean.length
        ? `${clean.length} ${plural(clean.length, "discipline")} ${clean.length === 1 ? "was" : "were"} ` +
          "read end to end and returned nothing to fix" +
          (clean.length > 4
  ...
  out.push({
    ...
    body: trimmed(sentences(
      ...
      corroborated
        ...
        : "Every finding rests on one reading and is marked asserted, so each should be checked " +
