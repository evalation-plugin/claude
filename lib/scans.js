...
const PHASES = {
  ...
  secret: "secret scan",
  history: "authorship in git history",
};
...
function phasesRead(packs) {
  ...
  for (const pack of packs) {
    // A concern set reads every tool's phase, and git history only where an item asks for it, since
    // a result nothing asks about is a card no concern rates.
    if (pack.kind === "concern-set") Object.keys(PHASES).filter((one) => one !== "history").forEach((one) => read.add(one));
...
/**
 * The card a scanner result is listed on: one per package for a dependency, whatever versions of it
 * the lockfile holds, since one version clears all, and one per rule otherwise.
 */
function cardOf(one) {
...
/** The worst severity on the card a result is listed on, from every result of that scan. */
...
module.exports = { PHASES, SCANS, cardOf, cardWorst, compared, compatible, named, newestFor, phasesRead, said, upgradeTo, whereOf };
